...
fprintf(fileID,F,'time [s]','theta [rad]', 'psi [rad]', ...
    ...
    'l [N.m]', 'm [N.m]', 'n [N.m]', 'u_r [m/s]', 'ax [m/s^2]', 'ay [m/s^2]', 'az [m/s^2]');

A = [t' att(:,2) att(:,3) att(:,1) ang_v(:,2) ang_v(:,3) ang_v(:,1) ...
    pos(:,1:2) -pos(:,3) v Va alpha beta delta(:,2) delta(:,1) delta(:,4) ...
    delta(:,3) F_aero F_body T_aero T_body u_r accel(:,1) accel(:,2) accel(:,3)];
...
str4 = ' %1.4f %3.4f %3.4f %3.4f\n';
fprintf(fileID,strcat(str1,str2,str3,str4),A);
fclose(fileID);