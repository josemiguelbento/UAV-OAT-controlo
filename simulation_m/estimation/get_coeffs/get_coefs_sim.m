...
%[data] = read_data('txt_logs_wAeroAccel\2022-12-12_22.04_recrutas.txt');
% data_t = struct2table(data);
%data_t{:,:} = data_t{:,:} + wgn(size(data_t,1), size(data_t,2), -1); % 10^-5
...
% least squares (without weights)
[CY_coeffs_ls] = ls1(X_lat, CY');
[Cl_coeffs_ls] = ls1(X_lat, Cl');
[Cn_coeffs_ls] = ls1(X_lat, Cn');
