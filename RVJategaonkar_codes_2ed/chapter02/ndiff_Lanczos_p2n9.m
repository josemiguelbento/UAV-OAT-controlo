...
function y = ndiff_Lanczos_p2n9(x, Nzi, izhf, dtk) 
...
for kzi=1:Nzi,               % Nzi-loop for multiple time segments
    ...
    for kk=iIni:iEnd,         % time-point loop for a particular time segment
        ...
        elseif (kk <= iIni+4  |  kk >= iEnd-4 ),
            ...
            % (0.01666666666667; 0.03333333333333; 0.05; 0.06666666666667)
            hdum(kk) = ( + 0.06666666666667*( x(kk+4) - x(kk-4) )...
                         + 0.05000000000000*( x(kk+3) - x(kk-3) )...
                         + 0.03333333333333*( x(kk+2) - x(kk-2) )...
                         + 0.01666666666667*( x(kk+1) - x(kk-1) ) ) /dtk;
            % hdum(kk) = ( + 4.0*( x(kk+4) - x(kk-4) )...
            %              + 3.0*( x(kk+3) - x(kk-3) )...
            %              + 2.0*( x(kk+2) - x (kk-2) )...
            %              +     ( x(kk+1) - x(kk-1) ) ) /60.0 /dtk;
            % hdum = 0;
            % for i=1:m
