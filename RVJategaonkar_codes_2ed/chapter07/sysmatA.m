function Amat = sysmatA(state_eq, Nxp, Nx, Nparam, NparID, x, delx, u, param, parFlag, ts)
...
for ix=1:Nxp
    ...
    xdt3(1:Nx) = feval(state_eq, ts, xin, u, param);

    xin(ix) = zip1 - delx(ix);
    param(find(parFlag~=0)) = xin(Nx+1:Nx+NparID);
    xdt4(1:Nx) = feval(state_eq, ts, xin, u, param);
