function [ytSave, xhSave, xhstdD] = ukf_mod_augmented(state_eq, obser_eq,...
                                        Z, Uinp, Ndata, dt,...
...
for k=1:Ndata,
    ...
    if k > 1,
    u1p = Uinp(k-1,1:Nu)';                        % Exogenous control input vector 
        for ip=1:Nsp,
           param(find(parFlag~=0)) = Xsp(Nx+1:Nx+NparID,ip);
           xNoise(1:Nx,1)          = Xsp(Nxp+1:Nxp+Nx,ip);
           xSP = ruku4_augNoise(state_eq, dt, Xsp(1:Nxp,ip), xNoise, dt, u1p, u1c,...
                                                           param, Nx, Nxp, Nparam);
           xtSigPt(:,ip) = xSP;       
        end
    end
    ...
    for ip=1:Nsp,
        ...
        ySP = feval(obser_eq, dt, xtSigPt(1:Nxp,ip), u1c, param, yNoise);
        ytSigPt(:,ip)  = ySP;  
    end
