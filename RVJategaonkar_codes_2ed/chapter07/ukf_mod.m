function [ytSave, xhSave, xhstdD] = ukf_mod(state_eq, obser_eq, Z, Uinp, Ndata, dt,...
...
for k=1:Ndata,
    ...
    if k > 1,
        ...
        for ip=1:Nsp, 
            ...
            xSP = ruku4_aug(state_eq, dt, xt, dt, u1p, u1c, param, Nx, Nxp, Nparam);
    ...
    for ip=1:Nsp,
        xt  = xtSigPt(:,ip);
        param(find(parFlag~=0)) = xt(Nx+1:Nx+NparID);
        ySP = feval(obser_eq, dt, xt, u1c, param); % Call observation equation
