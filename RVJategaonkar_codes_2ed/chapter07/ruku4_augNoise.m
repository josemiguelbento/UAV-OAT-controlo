function xa = ruku4_augNoise(state_eq, ts, xa, xNoise, dt, u1, u2, param, Nx, Nxp, Nparam)

% Integration of augmented state equations by Runge-Kutta 4th order.
% *** version for augmented UKF with noise variables as states ****
%
...
xadt(1:Nx) = feval(state_eq, ts, xa, u1, param, xNoise);
rk1 = xadt*dt2;

u12 = ( u1 + u2 )/2;                             % average of u(k) and u(k+1) 
xa1 = xa + rk1;
% param(1:Nparam,1) = xa1(Nx+1:Nx+Nparam);       % see foregoing comment
xadt(1:Nx) = feval(state_eq, ts, xa1, u12, param, xNoise);
rk2 = dt2*xadt;
...
xa1 = xa + 2*rk3;
% param(1:Nparam,1) = xa1(Nx+1:Nx+Nparam);       % see foregoing comment
xadt(1:Nx) = feval(state_eq, ts, xa1, u2, param, xNoise);
rk4 = dt2*xadt;
