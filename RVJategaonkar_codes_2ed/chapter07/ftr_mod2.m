function [tFTR, sxftr, sxftrstd] = ftr_mod2(Z, X, Unip, dt, Uprate, OmegaHz,...
                                           Ny, Nu, Nx, Ndata);
...
%
% 08-March-2014
% Modified version of ftr_mod: Z and X instead of Z, (i.e. separate
% outputs Z and separate state X variables).
%
% Inputs:
%    Z             measured output variables (Ndata,Ny)
%    X             measured state variables (Ndata,Nx); Nx=Ny assumed
...
for it=2:Uprate:Ndata
    ...
    eexp = eexp.*fact;
    Z_w = Z_w + (ones(nw,1)*y(it,:)) .* (eexp*ones(1,Ny));
