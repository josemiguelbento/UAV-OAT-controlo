...
%----------------------------------------------------------------------------------------
% Model definition
state_eq   = 'xdot_TC33_attas_lat';     % function for state equations       
obser_eq   = 'obs_TC33_attas_lat';      % function for observation equations
Nx         = 2;                         % Number of states 
Ny         = 5;                         % Number of observation variables
Nu         = 3;                         % Number of input (control) variables
NparSys    = 15;                        % number of system parameters
Nparam     = NparSys + Ny;              % Total number of system and bias parameters
...
fltDataTS1 = fAttasAilRud1;
fltDataTS2 = fAttasAilRud1;
data       = [fltDataTS1; fltDataTS2];  

% Number of data points Ndata and cumulative index izhf
Ndata1 = size(fltDataTS1,1);
Ndata2 = size(fltDataTS2,1);
...
% param_min(3) = -6.35;                  % test case with Lda limited 
% param_max(3) = -3.00;
...
x0(:,1) = [0; 0];       % Linear model with lumped bias parameters: Initial conditions
x0(:,2) = [0; 0];       % reduce to zero, see Section 3.5.2, Eq. (3.29)
                        % and kept fixed (i.e., all parFlagX0 should be setto zero.)
