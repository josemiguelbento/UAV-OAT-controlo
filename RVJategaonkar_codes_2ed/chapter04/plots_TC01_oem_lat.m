function T = plots_TC01_oem_lat(T, Z, Y, Uinp, params, par_std, iter);
...
subplot(818),plot(T,Uinp(:,3)*r2d);                grid; ylabel('{\beta}  (°)');
xlabel('Time in sec');  
...
if  iter > 0, 
    ...
    subplot(524);errorbar(iterations, params(7,:),par_std(7,:));ylabel('Nr');         xlabel('iteration #');grid
    subplot(525);errorbar(iterations, params(3,:),par_std(3,:));ylabel('L{\delta}a'); xlabel('iteration #');grid
    subplot(526);errorbar(iterations, params(8,:),par_std(8,:));ylabel('N{\delta}a'); xlabel('iteration #');grid
    subplot(527);errorbar(iterations, params(4,:),par_std(4,:));ylabel('L{\delta}r'); xlabel('iteration #');grid
    subplot(528);errorbar(iterations, params(9,:),par_std(9,:));ylabel('N{\delta}r'); xlabel('iteration #');grid
    subplot(529);errorbar(iterations, params(5,:),par_std(5,:));ylabel('L{\beta}');   xlabel('iteration #');grid
    subplot(5,2,10);errorbar(iterations, params(10,:),par_std(10,:));ylabel('N{\beta}'); xlabel('iteration #');grid
end